...
const SongId=ref(0);
const ShowUpdataSongPage=ref(false);

const UpdateSongData=(index)=>{
  SongId.value=Songs.value[index].id;
}
</script>

<template>
  <div class="w-full h-full" v-if="ShowUpdataSongPage">
    <Admin_Update_Song_Page v-model:token="token" v-model:SongId="SongId"></Admin_Update_Song_Page>
  </div>
  <div class="overflow-x-auto mx-4" v-if="!ShowUpdataSongPage">
    <table class="table">
      <!-- head -->
      <thead>
      <tr>
      ...
      <tr class="text-white rounded-md"
        ...
        <td>
          <div class="inline pr-4" @click="DeleteSong(index)">
            <svg class="icon fill-red-700 inline" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg"
                 width="24" height="24">
              <path
          ...
          <svg class="icon fill-white inline" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg"
               width="24" height="24" @click="UpdateSongData(index)">
            <path
